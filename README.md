...
| <a id="k053"></a>K053 `data.types` | 数値・日付の型宣言。省略列は文字列、形からの型推測なし。表示だけの書式はscreenのvalue.formatで指定します。calculate/aggregate/selectで作った列にも宣言でき、その値は更新の実行時に検査して違反があれば止まります。XLSXの日付セルはdateの宣言でformatの文字列に変換します。 |
