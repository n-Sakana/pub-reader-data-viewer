...
| 操作 | いつ追記するか | 内容の例 |
|---|---|---|
| 作成 | 台帳が無い状態で更新ジョブが台帳を新しく作ったとき | `更新: 追加 990 件、更新 0 件、削除 0 件、未処理に戻した 0 件` |
| 更新 | 更新ジョブ（merge / replace 等）が共有台帳を置き換えたとき | 同上。先頭はdata.jobsのname、戻した件数の状態名はscreen.workStateのinitial |
| 削除 | 削除ジョブが共有台帳を置き換えたとき | `削除: 削除 4 件` |
| 送信 | 送信で確認状態を共有台帳へ書いたとき | `処理済 12 件、未処理 3 件`（状態名はscreen.workState.statesのtext） |
...
| <a id="k053"></a>K053 `data.types` | 数値・日付の型宣言。省略列は文字列、形からの型推測なし。表示だけの書式はscreenのvalue.formatで指定します。calculate/aggregate/selectで作った列にも宣言できます。XLSXの日付セルはdateの宣言でformatの文字列に変換します。 |
