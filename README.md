...
結果JSONは次の形です。保存値は文字列のままで、例えば`"001"`を`1`にはしません。
...
成功はexit 0、設定・入力・実行エラーは非0です。理由は標準エラー、除外の通知と要約は標準出力へ出します。既存の出力・入力・台帳・設定等への上書きは拒否します。毎回新しい`-Output`を指定してください。
...
3種類の日付書式はこのように別の列へ指定できます。1列に指定できる書式は1つです。**同じ列に複数の日付書式が混在する場合の自動判別はありません。** `data.types`は検証と数値・日付の絞り込みのための宣言です。見せる形だけを変える`screen`の`value.format`とは別です。

XLSX入力は、ブックで最初に列挙されたワークシートを読みます。任意のシート名を指定する機能はありません。数式は保存済みの計算値を読み、未計算・エラーセルは拒否します。再計算、Excelの見た目の再現、日付シリアル値から任意書式への変換はしません。入力元で先頭ゼロが失われた番号を復元することもできません。
...
- 任意シート選択、Excel数式の再計算、外部で消えた先頭ゼロの復元、ブックの装飾の保存。
