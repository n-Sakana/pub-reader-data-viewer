...
| `inputs[]` | 更新ジョブが読む各入力のID、ファイル、採用行数、キー空の除外数、重複除外数 |
...
| `warnings` | 空キー・完全重複の除外、0件の入力等の説明 |
...
`-ValidateOnly`は同じ検査段階のエラーを集め、先頭に`FAIL 4 errors`のように件数を出します。JSON構文、設定定義、入力ファイル、列・型、ジョブ準備の順で進み、その先を安全に検査できない段階で止めます。`STOP`は止まった段階、`NOT CHECKED`は未検査の範囲です。出ているエラーをまとめて直してから再実行してください。後の段のエラーが次回初めて出る場合があります。`-RunUpdate`は実行を続けられない最初のエラーで止まります。
...
`data.encoding`の省略値は`utf-8`。**`data.tables.<ID>.encoding`があればその表だけ上書きし、なければ共通設定を使います。** `shift_jis`（WindowsのCP932）、`utf-8`、`utf-16`等を指定できます。BOMと指定が違えば、実際と期待した文字コード、変更すべき設定パスを示して拒否します。BOMで黙って別の文字コードへ切り替えたり、BOMなしのファイルを自動推測したりはしません。XLSXの文字コードはこの設定の対象外です。
...
| 型宣言のある列の空セル | 空のまま型検証を続行。**計算・sumに渡すと0を仮定せずエラーになる場合がある**。空欄を集計に含めるかは手順で決める |
...
重複キーのエラーが出たら、複数の明細が正常なデータなのかを先に確認します。正常なら、明細を区別する連番・日付等も含めて入力のkeyを決め、**全件を保持して読んだあと**に`aggregate/groupBy`で照合したい単位へ集めます。入力のキー検査は集計より先です。「集計するので重複を許す」という省略指定はなく、distinctでは合算されません。[入力行と結合の単位](#join-grain)も参照してください。

キーの制御文字、空・重複した見出し、不正な引用符、列数の違い、不正な文字コードのバイト列はエラーです。非キーのタブ等の一部制御文字は警告して`?`へ置換し、引用符内のCR/LFは保持します。意味が壊れる入力を、別の正常値へ推測して読み替えません。

**重複見出しは、使わない列でも入力表に登録できません。** 例は`id,value,value`、前後の空白を除くと同じになる`value`と`value `です。列を番号で指定する方法や、読取り前に列を選別・改名する設定はありません。`data.labels`は表示名、`select`は読取り後の操作なので回避策にはなりません。元データの管理者へ、未使用の重複列なら削除、必要な列なら`value1/value2`等へ改名を依頼してください。改名した列は`data.tables.<ID>.key`と各列参照も合わせます。**元データを変更できず、重複列のある表が要件に必要なら、このアプリの設定だけではできません。** 表を外したJSONを完成品として返さないでください。
...
- 日付形式やBOMなし文字コードの自動推測、同じ列で複数日付形式を試す機能。
- 登録表にない派生列を、その名前のまま任意の台帳列へ増設すること。
...
出荷設定の425箇所を一覧にしています。415箇所は内容を設定できる領域（307個のスカラー値と108個の入れ物）です。独立した調整項目が415個あるという意味ではありません。内部名1箇所と互換項目9箇所も、効かない設定を調整しないよう明記します。追加の表・列・状態には同じ規則を繰り返します。新しい表別encoding・複合キー等は本文の該当節を参照。
...
| <a id="k004"></a>K004 `paths.ledger` | 複数PCで共有する1本の.xlsx。相対パスはアプリ基準。入力・ログと同じファイルにはできません。省略data/ReaderDataViewer-Ledger.xlsx。 |
...
| <a id="k015"></a>K015 `watch.targets[0].enabled` | この監視先を使うか。falseでも定義は保存されます。 |
| <a id="k016"></a>K016 `watch.targets[0].name` | 監視先を人が見分ける表示名。窓を探す条件はwindowの中へ。 |
| <a id="k017"></a>K017 `watch.targets[0].window` | 対象のトップレベル窓を絞る条件。別のアプリを拾わないようprocessName等を指定。 |
| <a id="k018"></a>K018 `watch.targets[0].window.className` | UI Automationが公開するクラス名。省略ならこの属性で絞りません。 |
| <a id="k019"></a>K019 `watch.targets[0].window.processName` | 対象プロセス名、.exeなし。省略ならこの属性で絞りません。 |
| <a id="k020"></a>K020 `watch.targets[0].window.scope` | 互換項目。トップレベルの窓だけを探すため、この位置のscopeは動作に影響しません。省略可。path/fieldのscopeは有効。 |
| <a id="k021"></a>K021 `watch.targets[0].path` | 窓から対象欄の親までの経路。[]ならwindowから直接fieldを探します。 |
| <a id="k022"></a>K022 `watch.targets[0].field` | 値を読む対象欄の条件。設定画面の選択機能で実際の属性を取得できます。 |
| <a id="k023"></a>K023 `watch.targets[0].field.automationId` | UI Automationの識別子。省略ならこの属性で絞りません。 |
| <a id="k024"></a>K024 `watch.targets[0].field.controlTypes` | UIAの種類、例Edit/Document/List。[]または省略なら種類を限定しません。 |
| <a id="k025"></a>K025 `watch.targets[0].field.scope` | descendants（省略時）は子孫全部、childrenは直接の子だけ。深い欄をchildrenにすると見つかりません。 |
| <a id="k026"></a>K026 `watch.targets[0].read` | valueはValuePattern、textはTextPattern、nameは表示名を読みます。対象欄が提供する方式を選択。 |
...
| <a id="k066"></a>K066 `data.types["A.a_amount"].type` | date / number / text。dateはformat必須。numberは全角数字・¥・桁区切り・括弧マイナスを読めます。空セルは型検証を止めません。 |
| <a id="k067"></a>K067 `data.types["B.b_qty"]` | この実在する入力列の型宣言。ラベルではなく表ID.見出しで指定。番号の先頭ゼロを保つ列はtextまたは省略。 |
| <a id="k068"></a>K068 `data.types["B.b_qty"].type` | date / number / text。dateはformat必須。numberは全角数字・¥・桁区切り・括弧マイナスを読めます。空セルは型検証を止めません。 |
| <a id="k069"></a>K069 `data.types["B.b_total"]` | この実在する入力列の型宣言。ラベルではなく表ID.見出しで指定。番号の先頭ゼロを保つ列はtextまたは省略。 |
| <a id="k070"></a>K070 `data.types["B.b_total"].type` | date / number / text。dateはformat必須。numberは全角数字・¥・桁区切り・括弧マイナスを読めます。空セルは型検証を止めません。 |
| <a id="k071"></a>K071 `data.types["C.c_price"]` | この実在する入力列の型宣言。ラベルではなく表ID.見出しで指定。番号の先頭ゼロを保つ列はtextまたは省略。 |
| <a id="k072"></a>K072 `data.types["C.c_price"].type` | date / number / text。dateはformat必須。numberは全角数字・¥・桁区切り・括弧マイナスを読めます。空セルは型検証を止めません。 |
| <a id="k073"></a>K073 `data.types["C.c_stock"]` | この実在する入力列の型宣言。ラベルではなく表ID.見出しで指定。番号の先頭ゼロを保つ列はtextまたは省略。 |
| <a id="k074"></a>K074 `data.types["C.c_stock"].type` | date / number / text。dateはformat必須。numberは全角数字・¥・桁区切り・括弧マイナスを読めます。空セルは型検証を止めません。 |
...
| <a id="k113"></a>K113 `data.jobs[0].steps[0].operation` | 実行する一般操作。join/extract/delete/append/update/merge/replace/select/calculate/aggregate/sort/distinct。操作ごとに使える項目が異なります。 |
...
| <a id="k119"></a>K119 `data.jobs[0].steps[1].operation` | 実行する一般操作。join/extract/delete/append/update/merge/replace/select/calculate/aggregate/sort/distinct。操作ごとに使える項目が異なります。 |
...
| <a id="k125"></a>K125 `data.jobs[0].steps[2].operation` | 実行する一般操作。join/extract/delete/append/update/merge/replace/select/calculate/aggregate/sort/distinct。操作ごとに使える項目が異なります。 |
...
| <a id="k149"></a>K149 `data.jobs[1].steps[0].operation` | 実行する一般操作。join/extract/delete/append/update/merge/replace/select/calculate/aggregate/sort/distinct。操作ごとに使える項目が異なります。 |
...
| <a id="k155"></a>K155 `data.jobs[1].steps[1].operation` | 実行する一般操作。join/extract/delete/append/update/merge/replace/select/calculate/aggregate/sort/distinct。操作ごとに使える項目が異なります。 |
...
| <a id="k161"></a>K161 `data.jobs[1].steps[2].operation` | 実行する一般操作。join/extract/delete/append/update/merge/replace/select/calculate/aggregate/sort/distinct。操作ごとに使える項目が異なります。 |
...
| <a id="k166"></a>K166 `data.jobs[1].steps[3].operation` | 実行する一般操作。join/extract/delete/append/update/merge/replace/select/calculate/aggregate/sort/distinct。操作ごとに使える項目が異なります。 |
...
| <a id="k172"></a>K172 `data.ledger.identity` | 台帳の各行を識別する列参照。1列の文字列または配列。ある表のkey全列と同じ順で指定し、sourceにも全部残します。一意で空欄なし。 |
