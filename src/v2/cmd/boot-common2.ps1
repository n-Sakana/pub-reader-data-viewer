# ---------------------------------------------------------------------------
# boot-common2.ps1 -- shared bootstrap for the two one-to-many .cmd builds.
#
# Same job as src\cmd\boot-common.ps1, kept separate for the same reason the
# rest of src\v2 is: the 1:1 sources are frozen so the existing distributables
# stay reproducible from them. The differences are the default data directory
# (data-100k, not data) and the message that names gen_data2.ps1.
#
...
function Rdv-DataDir([string[]] $tokens, [string] $here) {
  ...
  foreach ($t in $tokens) {
    ...
    if ($t -eq '-log' -or $t -eq '-autopick') { $skip = $true; continue }
    if ($t -notlike '-*') { [void]$cand.Add($t) }
  }
  if ($env:RDV_DATA) { [void]$cand.Add($env:RDV_DATA) }
  [void]$cand.Add((Join-Path (Split-Path -Parent $here) 'data-100k'))
  [void]$cand.Add((Join-Path $here 'data-100k'))
  foreach ($c in $cand) {
...
if (-not $RdvData) {
  Rdv-Fail "tableA.csv / tableB.csv / tableC.csv were not found. Generate them first:`r`n  powershell -File build\gen_data2.ps1`r`nor pass the data directory:`r`n  $($env:RDV_SELF) <dataDir>"
}
