...
// it at startup (saved content + processed flags) and rewrites it after an
// approved update or a "processed" mark.
//
...
public static class Rdv3Xlsx
{
    ...
    // ---- write -------------------------------------------------------------
    public static void Write(string path, string[] lines, bool[] processed, string runId)
    {
        ...
        try
        {
            ...
            using (ZipArchive z = new ZipArchive(fs, ZipArchiveMode.Create))
            {
                ...
                using (StreamWriter w = new StreamWriter(e.Open(), new UTF8Encoding(false), 1 << 16))
                {
                    ...
                    w.Write("<row>");
                    Cell(w, Rdv3XlsxHead.ProcessedHead, true);
                    for (int c = 0; c < Rdv3Ledger.ContentCols; c++) { Cell(w, Rdv3Ledger.ContentHead[c], true); }
                    w.Write("</row>");

                    for (int i = 0; i < lines.Length; i++)
                    {
                        w.Write("<row>");
                        Cell(w, processed[i] ? Rdv3Ledger.ProcessedTrue : Rdv3Ledger.ProcessedFalse, false);
                        string line = lines[i];
                        int p = 0;
                        int n = line.Length;
                        for (int c = 0; c < Rdv3Ledger.ContentCols; c++)
                        {
    ...
    // ---- read --------------------------------------------------------------
    public static void Read(string path, out string[] lines, out bool[] processed)
    {
        ...
        using (ZipArchive z = new ZipArchive(fs, ZipArchiveMode.Read))
        {
            ...
            List<bool> outProc = new List<bool>(131072);
            string[] cells = new string[Rdv3Ledger.ContentCols + 1];
            ...
            using (XmlReader xr = XmlReader.Create(sheet.Open()))
            {
                ...
                while (xr.Read())
                {
                    ...
                    if (xr.NodeType == XmlNodeType.EndElement && xr.LocalName == "row" && inRow)
                    {
                        ...
                        if (!sawHeader)
                        {
                            CheckHeader(cells, path);
                            sawHeader = true;
                            continue;
                        }
                        bool p = string.Equals(cells[0], Rdv3Ledger.ProcessedTrue, StringComparison.OrdinalIgnoreCase);
            ...
            processed = outProc.ToArray();
        }
    }

    private static void CheckHeader(string[] cells, string path)
    {
        bool ok = string.Equals(cells[0], Rdv3XlsxHead.ProcessedHead, StringComparison.Ordinal);
        for (int c = 0; ok && c < Rdv3Ledger.ContentCols; c++)
        {
            if (!string.Equals(cells[c + 1], Rdv3Ledger.ContentHead[c], StringComparison.Ordinal)) { ok = false; }
        }
        if (!ok)
        {
            throw new InvalidDataException("ledger header row does not match the expected 29 columns: " + path);
        }
    }
...
public static class Rdv3XlsxHead
{
    // the one non-ASCII string the ledger file itself carries; kept out of
    // Rdv3Text so the ledger format does not depend on the UI string table
    public const string ProcessedHead = "処理済み";     // 処理済み
}
