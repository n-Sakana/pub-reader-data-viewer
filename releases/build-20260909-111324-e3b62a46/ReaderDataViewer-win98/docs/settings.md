...
台帳は全PCから同じ共有ファイルへ向け、ログはPCごとに分けます。入力・設定・プログラム・ログを台帳や出力で上書きする配置にはできません。共有先の別名、ハードリンク、シンボリックリンクの完全な同一性判定はしません。同じファイルへ別名でアクセスする構成を避けてください。台帳と同じフォルダーには端末別の操作ログ`<台帳名>-操作ログ-<端末名>.csv`が追記されます（READMEの「端末別の操作ログ」）。
