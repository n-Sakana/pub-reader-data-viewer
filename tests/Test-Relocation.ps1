...
if(-not $Evidence){$Evidence=Join-Path $Root ('work/relocation-'+[Guid]::NewGuid().ToString('N'))}
...
$files=@(Get-RdvSourceFiles -Root $Root)+@(Get-Item -LiteralPath (Join-Path $PSScriptRoot 'Relocation.cs'))
...
[Relocation]::Run($Root,[IO.Path]::GetFullPath($Evidence))
