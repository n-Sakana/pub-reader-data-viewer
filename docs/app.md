...
### 起動用コンソールについて (現状: 出したまま)
`.cmd` を Explorer から起動すると、**こちらのコードが動く前に** Windows がコンソールを作る。
したがって .cmd が入口である限りゼロにはできず、できるのは寿命を縮めることだけ。実測:

| 方法 | 可視時間 |
|---|---|
| 出したまま (現行) | セッション中ずっと |
| PowerShell で子を CreateNoWindow 起動して親は即終了 | 224→535 ms |
| wscript (コンソールを持たない) 経由で同じことをする | 197→334 ms |

**一瞬でも窓が出入りすると、その瞬間に入力中のテキストからフォーカスが外れて作業が飛ぶ**。
オーナー判断で「チラつくくらいなら出したままの方がよい」となったので、現行は出したまま。
ゼロにするには入口をコンソールを持たない実行体 (`.vbs` など) に変える必要があり、
配布物の形が変わるため未実施。
...
- 座標は原稿の実測表 (`Rdv3Geom.cs`) から引く。描画・コントロール配置・受け入れ検査の
...
- 受け入れ検査は**ウィンドウを表示せずに**製品ソースを描画して行う (336 通り)。
