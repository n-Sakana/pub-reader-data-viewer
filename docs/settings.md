...
```
ReaderDataViewer.cmd     プログラム
ReaderDataViewer.json    この設定
```

`.cmd` と同じフォルダーに置きます。**無くても動きます**（下に書いた既定値で動作し、
...
設定ダイアログの **「画面を調べる」** を押すと、生きている UI Automation ツリーが出ます。
対象アプリを開いた状態で「再取得」→ 欄を選ぶ→「この要素を対象にする」で、
ウィンドウ・中間パス・欄・値の取り方がそのまま設定に入ります。手で書くより確実です。
