...
判定ではなく作業の状態。台帳 xlsx の `store.column` 列に `stored` の文字列で保存する。ボタンの文言は
現在の状態 (`{state}`)、押すと現在の状態から出る遷移を C# が探し、`confirm` (省略可) を
モーダルで確認してから状態を変えて **即時保存** する。遷移が無ければ「このレコードはすでに{state}です」。
台帳の列に定義外の値が入っていたら、その値を赤系でそのまま表示し、遷移は拒否する。
引継ぎは内容列が全部一致するときだけ保存値をそのまま引き継ぎ、新規・変更行は `initial` になる。
...
- 検索: 入力が `search.pattern` に一致しないと通知「番号1 が形式 … に一致しません」。0 件は大きな番号だけ
  残り判定「—」。1 件は自動選択。N 件は候補一覧モーダルが開き、行を選ぶと閉じてレコードを表示、
...
→ 引継ぎ: 変えた行は初期に戻り、変えていない行は保持)、設定モーダル、終了、xlsx の中身。
