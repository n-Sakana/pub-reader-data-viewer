...
> **検証状況：Linux上のブラウザー試験14項目と静的・サンプル整合検査30項目は通過。WindowsでのC#コンパイル、WPF／WebView2起動、実際の複数PC・SMB共有試験は未実施です。本番台帳に置き換える前に、下記のWindows試験と共有試験を行ってください。**
...
`compile` はコンパイルのみ、`test` は隔離したサンプルを使うC#の回帰試験です。どちらも本番台帳を更新しません。ただし、この報告時点でWindows上での合否は未確認です。`build.bat` の引数省略は `compile` です。元READMEにあった `build.bat data` や未同梱のデータ生成・過去版復元機能は提供していません。
...
src/                          アプリC# 23ファイルと起動PowerShell
web/                          HTML、JavaScript、CSS
lib/                          元ZIPのWebView2 DLL・表示文書
data/                         元ZIPの入力CSVとログ（内容維持）
build.bat                     compile / test の入口
