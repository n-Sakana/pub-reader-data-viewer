...
#
#   data\tableA.csv    1,000 rows x 10 fields, key1 unique
#   data\tableB.csv    1,000 rows x 10 fields, key1 REPEATS, some key2 blank
#   data\tableC.csv    1,000 rows x 10 fields, key2 unique
#   data\delete.csv    paired key2,b_ref conditions for the delete example
#   data\expected.txt  the oracle: row counts, join checksum, candidate counts
#
...
#   pwsh -File build\gen_data2.ps1 -Rows 200 -OutDir data-tiny   -> build\out\data-tiny
# ============================================================================
[CmdletBinding()]
param(
  [int]    $Rows   = 1000,
...
# The shipped set is data\ itself; a named -OutDir goes under build\out\.
$outRoot = Join-Path $root 'build\out'
if ([string]::IsNullOrEmpty($OutDir)) { $OutDir = Join-Path $root 'data' }
if (-not [IO.Path]::IsPathRooted($OutDir)) { $OutDir = Join-Path $outRoot $OutDir }
