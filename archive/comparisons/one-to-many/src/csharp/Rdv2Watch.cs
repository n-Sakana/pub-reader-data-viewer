// ============================================================================
// Rdv2Watch.cs -- reading detection. A copy of src\csharp\RdvWatch.cs with the
// same rules, kept separate on purpose: the 1:1 sources are frozen so the
// existing distributables stay reproducible from them.
//
// reading detection. Polls a Notepad window through UI
...
public sealed class Rdv2Watch
{
    ...
    {
        if (s == null || s.Length != Rdv2Spec.KeyLen) { return false; }
    ...
    public bool Bind()
    {
        try
        {
            ...
            IntPtr fg = Rdv2NativeFg.GetForegroundWindow();
    ...
    private void Loop()
    {
        ...
        while (!stop)
        {
            ...
            {
                pendingPolls++;
                double held = Rdv2Clock.MsSince(pendingSince);
                if (held >= StableMs && IsKey(cand) && (cand != lastFired || sawEmpty))
                {
                    ...
                    if (OnConfirmed != null)
                    {
                        OnConfirmed(cand, Rdv2Clock.MsBetween(pendingSince, confirmAt), pendingPolls, confirmAt);
                    }
                }
            }
            Thread.Sleep(PollMs);
...
public static class Rdv2NativeFg
{
